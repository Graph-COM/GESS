...
class QMOF(InMemoryDataset):
    ...
    def __init__(self, root, data_config, shift_config, seed):
        ...
        self.targ_train_num = shift_config['restrict_TL_train']
        self.shift_type = 'concept_shift'
        self.atom_types = data_config['atom_types']
        self.measure = data_config['measure']  # bandgap
        self.setting = data_config['setting']
        if self.setting == 'Par-Label':
            self.setting = f'Par-Label_#{self.targ_train_num}'
    ...
    def process(self):
        ...
        if self.setting.split('_')[0] == "Par-Label":
            ...
            targ_train_idx = random.sample(range(4000, 6000), self.targ_train_num)
